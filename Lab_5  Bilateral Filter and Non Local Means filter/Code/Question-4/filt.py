...
def gaussian(im, range_sigma, spatial_sigma):
          ...
          for p_x in range(width):
              ...
              for i in range(-n, n):
                  for j in range(-n, n):
                      ...
                      g1 = np.exp( -((q_x - p_x)**2 + (q_y - p_y)**2) / (2 * spatial_sigma**2) )
                      
                      g2r = np.exp(-((im[p_y,p_x,0]- im[q_y,q_x,0])**2)/(2 * range_sigma**2))
                      g2g = np.exp(-((im[p_y,p_x,1]- im[q_y,q_x,1])**2)/(2 * range_sigma**2))
                      g2b = np.exp(-((im[p_y,p_x,2]- im[q_y,q_x,2])**2)/(2 * range_sigma**2))
